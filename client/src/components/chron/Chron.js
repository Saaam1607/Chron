...
function Chron() {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(
    CookieManager.generateHeader() !== undefined
  );
  ...
  function fetchData() {
    fetch(`/api/v1/grafici?arrowClick=${arrowclick}&isMonth=${isMonth}`, {
      method: "GET",
  ...
  const dataPie = {
    ...
    labels:
      tasso < 0
        ? ["Percentuale di produttività in meno"]
        : ["Percentuale di produttività in più"],
    datasets: [
      {
        ...
        backgroundColor:
          tasso < 0
            ...
            : ["rgba(0, 255, 0, 0.2)", "rgba(0, 255, 0, 0)"],
        borderColor:
          tasso < 0
            ? ["rgba(255, 99, 132, 1)", "rgba(255, 99, 132, 0)"]
        ...
        <div className="analitycs">
          ...
          <div className="graphic">
            <div className="chart">
              ...
              <div className="buttondiv">
                <button className="arrow-button" onClick={() => setArrowclick((prev) => prev - 1)}>
                  {" "}
                  {"<"}{" "}
                </button>
                {isMonth
                  ? `Mese da ${dataSessione[0]} al ${
                      dataSessione[dataSessione.length - 1]
                    }`
                  : `Settimana da ${dataSessione[0]} al ${
                      dataSessione[dataSessione.length - 1]
                    }`}
                <button className="arrow-button" onClick={() => setArrowclick((prev) => prev + 1)}>
                  {" "}
            ...
            <div className="stats">
              <p>
                Tempo Totale {isMonth ? "mensile" : "settimanale"} ={" "}
                {Math.floor(tempoTotale / 60)}:{tempoTotale % 60} <br />
                Media {isMonth ? "mensile" : "settimanale"}={" "}
                {Math.floor(media / 60)}:{media % 60} <br />
