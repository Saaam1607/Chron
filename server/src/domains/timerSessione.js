...
router.put('/', verificaAutenticazione, (req, res) => {
    try{
        ...
        if (!minuti || typeof minuti !== 'number' || !date || isNaN(Date.parse(date))) {
            return res.status(400).json({ success: false, message: 'Errore, parametri mancanti o non validi' });
        }
        ...
        res.status(500).json({success: false, message: "Errore interno durante il salvataggio della sessione"})
    }
});
