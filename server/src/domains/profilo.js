...
const htmlBody = require("fs").readFileSync(
  require("path").join(
    __dirname,
    "..",
    "components",
...
router.post("/login", bodyParser.json(), (req, res) => {
  GestoreDB.login(req.body.email, req.body.password)
    .then((esito) => {
      if (esito) {
        return GestoreDB.getIDfromEmail(req.body.email).then((esito) => {
        ...
        res
          ...
          .json({
            success: "false",
            message:
              "Autenticazione non riuscita, credenziali non corrispondenti o valide",
          });
      }
    })
    .catch((error) => {
      res
        .status(500)
        .json({ success: "false", message: "Errore durante l'autenticazione" });
    });
});
...
router.post("/registrazione", bodyParser.json(), (req, res) => {
  if (GestoreDB.controllaEsistenzaEmail(req.body.email)) {
    return res
      .status(409)
      .json({ success: "false", message: "Errore, email già utilizzata" });
  }
  if (
    ...
    return res
      .status(401)
      .json({ success: "false", message: "Errore, credenziali non valide" });
  }
...
router.get("/data", (req, res) => {
  if (req.id == undefined) {
    res
      .status(400)
      .json({ success: "false", message: "Errore, ID non trovato" });
  } else {
    GestoreDB.getDataFromID(req.id)
      ...
      .catch((error) => {
        res
          ...
          .json({
            success: "false",
            message: `Errore durante la lettura dei dati: ${error}`,
          });
      });
  }
});
...
router.post("/forgot-password", (req, res) => {
  ...
  if (email == undefined) {
    return res
      .status(400)
      .json({ success: "false", message: "Errore, email mancante!" });
  } else {
    GestoreDB.getDataFromEmail(email)
      .then((result) => {
        ...
        const formattedHtmlBody = htmlBody.replace(
          "{{passwordResetLink}}",
          recoveryLink
        );
      ...
      .catch((error) => {
        res
          ...
          .json({
            success: "false",
            message: `Errore durante il forgot-password: ${error}`,
          });
      });
  }
});
    ...
    GestoreDB.aggiornaPassword(id, password)
      ...
      .catch((error) => {
        res
          ...
          .json({
            success: "false",
            message: `Errore durante il reset della password: ${error}`,
          });
      });
  });
});
